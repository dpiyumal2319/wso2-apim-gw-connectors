...
public class AWSFederatedSubscriptionAgent implements FederatedSubscriptionAgent {
    ...
    public FederatedCredential regenerateCredential(String externalSubscriptionId) throws APIManagementException {
        // AWS doesn't have a simple "regenerate" for API Keys that returns a new value in one shot easily
        // without deleting/recreating or interacting with imported keys.
        // Simplest strategy: Delete old key, find necessary info (from where?), create new key.
        // BUT: regenerateCredential(String externalSubscriptionId) only gives us the ID.
        // We lack context (Subscription UUID, API UUID) to re-associate with Usage Plan.
        // Wait, the interface design usually implies we might need more context or we rely on the caller
        // to pass a request if the interface allowed it.
        // The interface is `FederatedCredential regenerateCredential(String externalSubscriptionId)`.

        // PROBLEM: We cannot implement "Create New" purely from "Old Key ID" because we don't know which Usage Plan
        // it was attached to (well, we could query it) and we don't know the exact naming convention (wso2_SUB_UUID)
        // just from a random ID (unless we store the name in the ID or lookup the key to get the name).

        // Strategy:
        // 1. Get API Key details to retrieve the Name (wso2_{subscriptionId}).
        // 2. Get Usage Plans associated with this key.
        // 3. Delete old key.
        // 4. Create new key with same Name.
        // 5. Associate with the same Usage Plan(s).

        try {
            // 1. Get Key Details
            GetApiKeyRequest getRequest =
                    GetApiKeyRequest.builder()
                    .apiKey(externalSubscriptionId)
                    .includeValue(true) // We only get masking usually
                    .build();
            GetApiKeyResponse apiKey = apiGatewayClient.getApiKey(getRequest);
            String name = apiKey.name();
            
            // 2. Get Usage Plans
            // We need to know which Usage Plan it belongs to.
            GetUsagePlansRequest getPlansRequest = GetUsagePlansRequest.builder()
                    .keyId(externalSubscriptionId)
                    .build();
            GetUsagePlansResponse plansResponse = apiGatewayClient.getUsagePlans(getPlansRequest);
            List<UsagePlan> usagePlans = plansResponse.items();

            if (usagePlans.isEmpty()) {
                 throw new APIManagementException("No Usage Plan found for existing key during regeneration.");
            }
            // Assume 1 Usage Plan per key as per our design
            String usagePlanId = usagePlans.get(0).id();

            // 3. Delete Old Key
            deleteSubscription(externalSubscriptionId);

            // 4. Create New Key
            CreateApiKeyRequest createNewRequest = CreateApiKeyRequest.builder()
                    .name(name)
                    .enabled(true)
                    .description(apiKey.description())
                    .build();
            CreateApiKeyResponse newKeyResponse = apiGatewayClient.createApiKey(createNewRequest);
            String newKeyId = newKeyResponse.id();
            String newKeyValue = newKeyResponse.value();
            ...
            credBody.addProperty("credentialType", CREDENTIAL_TYPE);
            credBody.addProperty("headerName", HEADER_NAME);
            credBody.addProperty("value", newKeyValue);

            FederatedCredential credential = new FederatedCredential();
            credential.setBody(credBody.toString());
            credential.setExternalSubscriptionId(newKeyId);
            credential.setValueRetrievable(false);
            credential.setMasked(false);

            return credential;

        } catch (Exception e) {
            throw new APIManagementException("Error regenerating credential on AWS: " + e.getMessage(), e);
        }
    }
